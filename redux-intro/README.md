...
#### First, Install Things and Create the App
...
```
Reducers go into the combineReducers object

#### What are Reducers?

This is a basic Reducer:
```
...
```
* The if statement controls whether or not that particular reducer fires off it's information; this is done to control what information goes in and out of the store.
* When logged, "state" returns an empty object.
* When logged, "action" will return an object with a "payload" and a "type".
  - "type" is a magic word that transfers the string for the if statement. Why? No idea.
  - "payload" is just a standard practice name that passes an object full of whatever information you want to send to the store.
* The name of the Reducer needs to be put into the store inside the "combineReducers" object.

#### Where does BUTTON_ONE come from and what does it look like?

```
<button onClick={this.handleClick('BUTTON_ONE')}>Button One!</button>
```
When clicking this button, "BUTTON_ONE" is passed into the "propertyName" below and triggers the appropriate Reducer's if function.
```
...
#### How do we get the information back into the component?
...
# All the Crap that was Auto Created Follows:
